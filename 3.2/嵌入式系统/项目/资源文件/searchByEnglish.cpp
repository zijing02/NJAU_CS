...
void MainWindow::searchByEnglish()
{
    QWidget *page8 = ui->stackedWidget->widget(8);
    QTableWidget *tableWidget = page8->findChild<QTableWidget *>("search");

    if (!tableWidget)
    {
        QVBoxLayout *layout = new QVBoxLayout(page8);
        ...
        connect(searchButton, &QPushButton::clicked, this, [this, searchLineEdit, tableWidget]()
            ...
            if (!searchText.isEmpty()) {
                performEnglishSearch(searchText, tableWidget);
            } });
...
void MainWindow::performEnglishSearch(const QString &searchText, QTableWidget *tableWidget)
{
    qDebug() << "正在搜索单词:" << searchText;

    tableWidget->blockSignals(true);
    tableWidget->setRowCount(0);

    QSqlQuery query(db);
    query.prepare("SELECT name, explanation FROM words WHERE name LIKE ? ORDER BY name");
