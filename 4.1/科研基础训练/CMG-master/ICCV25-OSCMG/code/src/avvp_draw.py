...
from model.main_model_2 import Semantic_Decoder_AVVP, AVT_VQVAE_Encoder, AVT_RQVAE_Encoder, AVT_MLVQVAE_Encoder
...
def main():
    ...
    if args.dataset_name == 'avvp_va':
        train_csv_path = 'data/AVVP/data/AVVP_eval_visual_checked_combined.csv'
        val_csv_path = 'data/AVVP/data/AVVP_eval_audio_checked_combined.csv'
        audio_fea_base_path = 'data/AVVP/feature/audio/zip'
        video_fea_base_path = 'data/AVVP/feature/video/zip'
        ...
        train_csv_path = 'data/AVVP/data/AVVP_eval_audio_checked_combined.csv'
        val_csv_path = 'data/AVVP/data/AVVP_eval_visual_checked_combined.csv'
        audio_fea_base_path = 'data/AVVP/feature/audio/zip'
        video_fea_base_path = 'data/AVVP/feature/video/zip'
        train_dataloader = DataLoader(
    ...
    # AVT
    Encoder = AVT_MLVQVAE_Encoder(audio_dim, video_dim, text_lstm_dim*2, audio_output_dim, video_output_dim, text_output_dim, n_embeddings, embedding_dim)
    
    Decoder = Semantic_Decoder_AVVP(input_dim=embedding_dim*2,class_num=26) #256对应embedding_dim，即codeword的维度
    Encoder.double()
    Decoder.double()
    Encoder.to(device)
    Decoder.to(device)
...
def train_epoch(Encoder, Decoder,ExpLogLoss_fn, train_dataloader, criterion, criterion_event, optimizer, epoch, total_step, args):
    ...
    codebook_count_1 = torch.zeros(400,26)#codebook_size,class_num
    codebook_count_2 = torch.zeros(400,26)#codebook_size,class_num
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        if (args.dataset_name == 'avvp_va'):
            ...
            with torch.no_grad():
                video_vq, indices, indices_2 = Encoder.Video_VQ_Encoder(feature)
            ...
            for i in range(B*T):
                # if(decoder_label[i]!=0):
                codebook_count_1[indices[i]][decoder_label[i]]+=1
                codebook_count_2[indices_2[i]][decoder_label[i]]+=1
            
        elif (args.dataset_name == 'avvp_av'):
            with torch.no_grad():
                audio_vq, indices, indices_2 = Encoder.Audio_VQ_Encoder(feature)
    ...
    tsne1 = TSNE(n_components=2, init='pca', learning_rate='auto')
    embedding = tsne1.fit_transform(Encoder.Cross_quantizer.embedding[0].cpu())
    
    tsne2 = TSNE(n_components=2, init='pca', learning_rate='auto')
    embedding_2 = tsne2.fit_transform(Encoder.Cross_quantizer.embedding[1].cpu())
    
    # print(codebook_count_1)
    
    catego_1 = torch.argmax(codebook_count_1, dim=-1)
    catego_2 = torch.argmax(codebook_count_2, dim=-1)
    
    # 绘制这些点
    plt.figure(figsize=(10, 8))

    # 为每个种类选择一个颜色，并绘制属于该种类的点
    # colors = plt.cm.rainbow(np.linspace(0, 1, 26))

    colors = [
    "#FF0000", "#FFA500", "#FFFF00", "#008000", "#0000FF", "#4B0082", "#EE82EE",
    "#800000", "#FF00FF", "#808000", "#00FF00", "#008080", "#00FFFF", "#000080",
    "#FFC0CB", "#A52A2A", "#000000", "#D2691E", "#5F9EA0", "#7FFF00", "#DC143C",
    "#00BFFF", "#F4A460", "#00008B", "#FFD700", "#ADFF2F"
    ]
    ...
    # ])
    # selected_colors = np.random.choice(range(combined_colors.shape[0]), 26, replace=False)
    # colors = combined_colors[selected_colors]
    
    for i in range(1,26):
        category_points = embedding[catego_1 == i]
        plt.scatter(category_points[:, 0], category_points[:, 1], color=colors[i], label=category_list[i-1], alpha=0.7)
    ...
    if (args.dataset_name == 'avvp_va'):
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/va_level1_epoch'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
    else:
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/av_level1_epoch'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
    ...
    # colors = plt.cm.rainbow(np.linspace(0, 1, 26))
    for i in range(1,26):
        category_points = embedding_2[catego_2 == i]
        plt.scatter(category_points[:, 0], category_points[:, 1], color=colors[i], label=category_list[i-1], alpha=0.7)
    ...
    # plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    if (args.dataset_name == 'avvp_va'):
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/va_level2_epoch'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
    else:
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/av_level2_epoch'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
...
def validate_epoch(Encoder,Decoder,ExpLogLoss_fn, val_dataloader, criterion, criterion_event, epoch, args, eval_only=False):
    ...
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    if (args.dataset_name == 'avvp_va'):
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/va_level1_epoch_eval'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
    else:
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/av_level1_epoch_eval'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
    ...
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    if (args.dataset_name == 'avvp_va'):
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/va_level2_epoch_eval'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
    else:
        plt.savefig('data/new-cvpr2024/code/src/codebook_visualization/avvp/av_level2_epoch_eval'+str(epoch)+'.png',dpi=400,bbox_inches = 'tight', pad_inches=0)
