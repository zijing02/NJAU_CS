...
from model.main_model_2 import Semantic_Decoder,AVT_RQVAE_Encoder,AVT_VQVAE_Encoder,AVT_MLVQVAE_Encoder
...
def main():
    ...
    train_dataloader = DataLoader(
        AVEDataset(data_root, split=str(args.source_class_num)),
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        if args.dataset_name == 'ave_va':
            with torch.no_grad():# Freeze Encoder
                ...
                # print('video_vq.shape:',video_vq.shape)
...
def validate_epoch(Encoder,Decoder, val_dataloader, criterion, criterion_event, epoch, args, eval_only=False):
    ...
    for n_iter, batch_data in enumerate(val_dataloader):
        ...
        if args.dataset_name == 'ave_va':
            output_sum.append(audio_class)
        else:
            output_sum.append(video_class)
